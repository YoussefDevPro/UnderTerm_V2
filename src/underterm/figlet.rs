/*
 * THIS IS A MODIFIED VERSION OF https://crates.io/crates/figlet-rs to support unicode chars and
 * the codetag of this freaking font
 * */
...
impl FIGfont {
    ...
    fn extract_one_line(
        ...
        // Previous code used byte-based slicing which breaks on multibyte (Unicode) characters.
        // Here we operate in terms of chars: remove the trailing end-markers (1 normally, 2 for last line when height != 1),
        // then replace hardblank with space. This avoids slicing on invalid UTF-8 boundaries.
        let char_count = line.chars().count();

        // number of trailing endmark chars to remove:
        let remove_count = if is_last_index && height != 1 { 2 } else { 1 };

        if char_count < remove_count {
            return Err(format!(
                "line too short to remove endmark(s) at index:{index}"
            ));
        }

        let keep_count = char_count - remove_count;
        let trimmed: String = line.chars().take(keep_count).collect();

        Ok(trimmed.replace(hardblank, " "))
    }
    ...
    fn read_codetag_font(
        ...
        // offset after header, comments, and the usual 102 glyphs (95 + 7)
        let offset = (1 + headerline.comment_lines + 102 * headerline.height) as usize;

        // If there's nothing after the required fonts, there are no codetags.
        if offset >= lines.len() {
            return Ok(());
        }
        ...
        // If codetag_count is explicitly provided:
        if let Some(count) = headerline.codetag_count {
            if count == 0 {
                // explicitly no codetags
                return Ok(());
            }
            let expected_lines = (count as usize) * codetag_height;
            if codetag_lines < expected_lines {
                return Err("codetag font is illegal: not enough lines".to_string());
            }
        ...
        // codetag_count not provided: infer from remaining lines
        if codetag_lines == 0 {
            return Ok(());
        }
    ...
    pub fn standard() -> Result<FIGfont, String> {
        let contents = std::include_str!("../../src/underterm/default.flf");
        FIGfont::from_content(contents)
    }
